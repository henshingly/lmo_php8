...
class liga {
  ...
  function &partieForTeams(&$heim,&$gast) {
    ...
    foreach ($this->partien as $aPartie) {
      if (($aPartie->heim == $heim ) && ($aPartie->gast == $gast)) {
        $result = $aPartie;
        break;
      }
    }
  ...
  function loadFile($fileName='') {
    ...
    if(file_exists($fileName) && $fileName <> '') {
      ...
      // Partien einlesen
      $rCounter = 1;
      while (isset($iniData["Round".$rCounter]) && $iniData["Round".$rCounter]!="" ) {
        $pCounter = 1;
        ...
        while (isset($iniData[$roundSektion]["TA".$pCounter]) && $iniData[$roundSektion]["TA".$pCounter]!="" ) {
          ...
          do {
            ...
            if (isset($heimTeam) && isset($gastTeam)) {
    ...
    foreach ($this->spieltage as $spieltag) {
      foreach ($spieltag->partien as $partie) {
        ...
        foreach ($tableArray as $table) { // Teams der Partie finden
          if ($table["team"]===$partie->heim) {
            $heimCount=$count;
          }
          if ($table["team"]===$partie->gast) {
            $gastCount=$count;
          }
          if ($heimCount>-1 && $gastCount>-1) {
            break;
          }
          $count++;
        }
