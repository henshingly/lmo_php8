...
if ($action == 'tipp') {
    ...
    if ($_SESSION['lmotipperok'] < 1 && $_SESSION['lmotipperok'] > -4) {
        ...
        if (!empty($xtippername) && !empty($xtipperpass)) {
            ...
            if (($tippers = file($pswfile)) === false)
                $tippers = array();
            $_SESSION['lmotipperok'] = -2;
            $updateFile = false; // NEU
            foreach ($tippers as $key => $tipper) {
                if ($_SESSION['lmotipperok'] == -2) {
                    $fileinfo = explode('|', trim($tipper));
                    if ($_SESSION['lmotippername'] == $fileinfo[0]) {
                        // Nick gefunden
                        $_SESSION['lmotipperok'] = -1;
                        $storedPass = $fileinfo[1];
                        $inputPass  = $_SESSION['lmotipperpass'];
                        $loginOk = false;
                        // Prüfen ob Hash vorhanden
                        if (password_get_info($storedPass)['algo'] !== null) {
                            if (password_verify($inputPass, $storedPass)) {
                                $loginOk = true;
                                // ggf. neu hashen (besserer Algo verfügbar)
                                if (password_needs_rehash($storedPass, $algo)) {
                                    $fileinfo[1] = password_hash($inputPass, $algo);
                                    $tippers[$key] = implode('|', $fileinfo) . "\n";
                                    $updateFile = true;
                                }
                            }
                        } else {
                            // Klartext → prüfen + migrieren
            ...
            if ($updateFile) {
                file_put_contents($pswfile, implode('', $tippers));
            }
        }
    }
