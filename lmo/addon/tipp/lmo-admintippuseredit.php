...
if ($action=="admin" && $todo=="tippuseredit" && ($nick!="" || $save==-1)) {
    ...
    // 1. DATEI EINLESEN & SCHUTZZEILE ENTFERNEN
    $users_raw = file_exists($pswfile) ? file($pswfile, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES) : array();
    $users = array();
    foreach ($users_raw as $line) {
        if (strpos($line, '<?php') !== false)
            continue;
        $users[] = $line;
    }
    array_unshift($users, ''); // LMO-typischer Leer-Index am Anfang
    ...
    if ($save != -1) {
        ...
        $tipp_tipperdaten = array_pad(array(), 12, '');
        $save = count($users);
    ...
    if ($newpage == 1) {
        ...
        // 2. PASSWORT-HASHING LOGIK
        if (trim($xtipperpass) != '') {
            $bestAlgo = defined('PASSWORD_ARGON2ID') ? PASSWORD_ARGON2ID : PASSWORD_BCRYPT;
            $tipp_tipperdaten[1] = password_hash(trim($xtipperpass), $bestAlgo);
        }
        // Wenn kein Passwort eingegeben wurde, bleibt der alte Wert (Hash oder Klartext) in $tipp_tipperdaten[1] stehen
    ...
    foreach($users as $line) {
        if (!empty($line)) $output_lines[] = $line;
    }
    $content = '<?php exit; ?>' . PHP_EOL . implode(PHP_EOL, $output_lines) . PHP_EOL;
    file_put_contents($pswfile, $content, LOCK_EX);

    // .tip Dateien verwalten
    ...
    <table class="lmoMiddle" cellspacing="0" cellpadding="0" border="0" width="50%">
      <tr><th class="lmoMenu"><?php echo $text['tipp'][114]; ?></th></tr>
      <tr><td align="center"><a href='<?php echo $_SERVER['PHP_SELF']; ?>?action=admin&amp;todo=tippuser'><?php echo $text[5] . ' ' . $text['tipp'][111] . ' ' . $text['tipp'][53]; ?></a></td></tr>
    </table><?php
  } else {
?>
  <table class="lmoMiddle" cellspacing="0" cellpadding="0" border="0">
    <tr>
      <td align="center"><h1><?php if ($save == count($users)) { echo $text['tipp'][136]; } else { echo $text['tipp'][106]; } ?></h1></td>
    </tr>
    <tr>
      <td align="center">
        ...
        if ($save == count($users)) { ?>
            ...
            <tr>
              <td align="right">&nbsp;<?php if ($save == count($users)) { echo $text[308]; } else { ?><acronym title="<?php echo $text['tipp'][304]; ?>"><?php echo $text[308]; ?></acronym><?php } ?></td>
              <td align="left"><?php
        if ($save == count($users)) { ?>
            ...
            <tr>
              <td align="right">&nbsp;<?php echo $text['tipp'][14]; ?></td>
              <td align="left"><input class="lmo-formular-input" type="text" name="xtippervorname" size="25" maxlength="32" value="<?php echo substr($tipp_tipperdaten[3], 0, strpos($tipp_tipperdaten[3], ' ')); ?>"></td>
            </tr>
            <tr>
              <td align="right">&nbsp;<?php echo $text['tipp'][15]; ?></td>
              <td align="left"><input class="lmo-formular-input" type="text" name="xtippernachname" size="25" maxlength="32" value="<?php echo substr($tipp_tipperdaten[3], strpos($tipp_tipperdaten[3], ' ') + 1); ?>"></td>
            </tr>
            <tr>
              <td align="right">&nbsp;<?php echo $text['tipp'][126]; ?></td>
              <td align="left"><input class="lmo-formular-input" type="text" name="xtipperstrasse" size="25" maxlength="32" value="<?php echo $tipp_tipperdaten[6]; ?>"></td>
            </tr>
            <tr>
              <td align="right">&nbsp;<?php echo $text['tipp'][127]; ?></td>
              <td align="left"><input class="lmo-formular-input" type="text" name="xtipperplz" size="7" maxlength="5" value="<?php echo $tipp_tipperdaten[7]; ?>"></td>
            </tr>
            <tr>
              <td align="right">&nbsp;<?php echo $text['tipp'][128]; ?></td>
              <td align="left"><input class="lmo-formular-input" type="text" name="xtipperort" size="25" maxlength="32" value="<?php echo $tipp_tipperdaten[8]; ?>"></td>
            </tr>
            <tr>
              <td align="right">&nbsp;<?php echo $text['tipp'][16]; ?></td>
              <td align="left"><input class="lmo-formular-input" type="text" name="xtipperemail" size="25" maxlength="64" value="<?php echo $tipp_tipperdaten[4]; ?>"></td>
            </tr>
            <tr>
              <td>&nbsp;</td>
...
<?php
        if ($tipp_tipperimteam >= 0) { ?>
            ...
            echo '<option value=" "';
            if ($xtippervereinalt == '') {echo ' selected';} echo '>' . $text['tipp'][51] . '</option>';
...
<?php
            ...
            <tr>
              <td class="lmost5" colspan="2"><?php
        $ftype = '.l98';
        require(PATH_TO_ADDONDIR . '/tipp/lmo-tippnewdir.php'); ?></td>
            </tr>
            <tr>
              <td align="left">
            ...
            <tr>
              <td class="lmoFooter" colspan="2" align="right"><?php echo '<strong>' . $text['tipp'][82] . '</strong> ' . $text['tipp'][137]; ?></td>
            </tr>
          </table>
        </form>
      </td>
