...
// 
  ...
  if($kmodus<4){
    for($i=0;$i<count($linie)-1;$i++){if(strlen($linie[$i])>0){if($kmodus==1){$linie[$i]=$max-$linie[$i];}else{$linie[$i]-=$min;}}}
    if($pganz==2){
      for($i=0;$i<count($lini2)-1;$i++){if(strlen($lini2[$i])>0){if($kmodus==1){$lini2[$i]=$max-$lini2[$i];}else{$lini2[$i]-=$min;}}}
      }
    }
  if($kmodus>2){
    for($i=0;$i<count($liniea)-1;$i++){if(strlen($liniea[$i])>0){$liniea[$i]-=$min;}}
    if($pganz==2){
      for($i=0;$i<count($lini2a)-1;$i++){if(strlen($lini2a[$i])>0){$lini2a[$i]-=$min;}}
      }
    }

  if(($max-$min)>0){$khoch=round(240/($max-$min));}
  else{$khoch=12;}
  
  if($khoch>12){$khoch=12;}
  if($khoch<1){$khoch=1;}
  elseif(($max-$min)<21){$khoch=12;}
  ...
  for($i=$min;$i<=$max;$i++){imagerectangle($image,29,28+(($i-$min)*$khoch),17+(($pgst+1)*12),28+$khoch+(($i-$min)*$khoch),$farbe_b);}
  for($i=0;$i<$pgst;$i++){imagerectangle($image,29+($i*12),28,41+($i*12),16+(($max-$min+1)*$khoch+12),$farbe_b);}
  $j=1;
  if($kmodus>1 && $khoch>1){
    for($i=$min;$i<=$max;$i++){
      if($i==1){for($k=1;$k<=$pgst;$k++){imagefill($image,20+($k*12),29+(($i-$min)*$khoch),$farbe_e);}}
      if($i==2){for($k=1;$k<=$pgst;$k++){imagefill($image,20+($k*12),29+(($i-$min)*$khoch),$farbe_f);}}
      if($i==3){for($k=1;$k<=$pgst;$k++){imagefill($image,20+($k*12),29+(($i-$min)*$khoch),$farbe_g);}}
      }
    }
