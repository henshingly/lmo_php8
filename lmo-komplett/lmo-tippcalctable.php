...
if($file!=""){
  if($m==0){
    ...
    for($i=0;$i<$anzst;$i++){$btip[$i] = array_pad(array("false"),$anzsp+1,"false");}
    }
  $stt=0;
  $hoy=0;
  for($a=1;$a<=$anzteams;$a++){
    if($tabtype==3){$hoy=($anzst/2);}
    if($tabtype==4){$endtab=($anzst/2);}
    for($j=$hoy;$j<$endtab;$j++){
      for($i=0;$i<$anzsp;$i++){
        if($m==0 && $a==1){ // nur beim ersten Tipper berechnen, ob das Spiel in die Tipp-Tabelle einfließen darf
          if($einsichterst==1){
            $plus=0;
            $btip[$j][$i]=tippaenderbar($mterm[$j][$i],$datum1[$j],$datum2[$j]);
            }
          elseif($einsichterst==2){
            if($goala[$j][$i]!="_" && $goalb[$j][$i]!="_"){$btip[$j][$i]=false;}
            else{$btip[$j][$i]=true;}
            }
          else{ // Tipps immer anzeigen
            $btip[$j][$i]=false;
            }
          if($mtipp[$j][$i]==1){ // nicht in der Wertung
            $btip[$j][$i]=true;
            }
          }
        if($btip[$j][$i]==false){
          ...
          if(($goaltippa[$j][$i]!="_") && ($goaltippb[$j][$i]!="_") && ((($tabtype==0 or $tabtype==3 or $tabtype==4) && (($a==$teama[$j][$i]) || ($a==$teamb[$j][$i]))) || (($tabtype==1) && ($a==$teama[$j][$i])) || (($tabtype==2) && ($a==$teamb[$j][$i])))){
            ...
            if($a==$teama[$j][$i]){
              ...
              if($goaltippa[$j][$i]>$goaltippb[$j][$i]){
                ...
                if($minus==2){$negativ[$a]=$negativ[$a]+$p0n;}
                }
              elseif($goaltippa[$j][$i]<$goaltippb[$j][$i]){
                $nieder[$a]=$nieder[$a]+1;
                $punkte[$a]=$punkte[$a]+$p0n;
                if($minus==2){$negativ[$a]=$negativ[$a]+$p0s;}
                }
              elseif($goaltippa[$j][$i]==$goaltippb[$j][$i]){
                $unent[$a]=$unent[$a]+1;
                $punkte[$a]=$punkte[$a]+$p0u;
                if($minus==2){$negativ[$a]=$negativ[$a]+$p0u;}
                }
              }
            if($a==$teamb[$j][$i]){
    ...
    if($tabtype==0 or $tabtype==3 or $tabtype==4){
      ...
      if($minus==2){$negativ[$a]=$negativ[$a]-$strafm[$a];}
      }
    if($m==($anztipper-1)){
      $dtore[$a]=$etore[$a]-$atore[$a];
      $quote[$a]=0;
      if($spiele[$a]!=0){$quote[$a]=number_format($punkte[$a]/$spiele[$a],2,".",",");}
      $quote[$a]*=100;
      if($kegel==0){array_push($tab0,(50000000+$quote[$a]).(50000000+$punkte[$a]).(50000000-$negativ[$a]).(50000000+$dtore[$a]).(50000000+$etore[$a]).(50000000+$a));}else{array_push($tab0,(50000000+$quote[$a]).(50000000+$punkte[$a]).(50000000-$negativ[$a]).(50000000+$etore[$a]).(50000000+$dtore[$a]).(50000000+$a));}
      }
      ...
      for($abc=1;$abc<$anzteams;$abc++){
        ...
        if(($x1!=$x2) || (($abc==$anzteams-1) && ($x1==$x2))){
          if($cba>1){
            $subteams="";
            for($b=1;$b<=$cba;$b++){
              if($b>1){$subteams=$subteams.".";}
              if(($abc==$anzteams-1) && ($x1==$x2)){$subteams=$subteams.intval(substr($tab0[$abc-$b+1],42));}else{$subteams=$subteams.intval(substr($tab0[$abc-$b],42));}
              }
            $anzcnt=0;
            require(PATH_TO_LMO."/lmo-tippcalctable1.php");
            if($anzcnt>0){
              for($b=1;$b<=count($tab1);$b++){
                for($f=0;$f<count($tab0);$f++){if(intval(substr($tab0[$f],42))==intval(substr($tab1[$b-1],42))){$tab0[$f]=substr($tab0[$f],0,17-strlen($b)).$b.substr($tab0[$f],17);}}
                }
              }
            }
          $cba=1;
