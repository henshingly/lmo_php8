...
if($file!=""){
  ...
  for($i=0;$i<$anzst;$i++){$taba[$i] = array("");}
  $platz = array_pad($array,$anzteams+1,"");
  for($i=0;$i<$anzteams;$i++){$platz[$i] = array("");$platz[$i] = array_pad($array,$anzst+1,"");}
  for($jyz=0;$jyz<$anzst;$jyz++){
    ...
    for($a=1;$a<=$anzteams;$a++){
      for($i=0;$i<$anzsp;$i++){
        if(($goala[$jyz][$i]!="_") && ($goalb[$jyz][$i]!="_") && ((($tabtype==0) && (($a==$teama[$jyz][$i]) || ($a==$teamb[$jyz][$i]))) || (($tabtype==1) && ($a==$teama[$jyz][$i])) || (($tabtype==2) && ($a==$teamb[$jyz][$i])))){
          ...
          $p0n=$pnn;
          if($spez==1){
            if($mspez[$jyz][$i]==$text[0]){$p0s=$pxs;$p0u=$pxu;$p0n=$pxn;}
            if($mspez[$jyz][$i]==$text[1]){$p0s=$pps;$p0u=$ppu;$p0n=$ppn;}
            }
          $spiele[$a]=$spiele[$a]+1;
    ...
    for($a=1;$a<=$anzteams;$a++){
      ...
      if($kegel==0){array_push($taba[$jyz],(50000000+$punkte[$a]).(50000000-$negativ[$a]).(50000000+$dtore[$a]).(50000000+$etore[$a]).(50000000+$a));}else{array_push($taba[$jyz],(50000000+$punkte[$a]).(50000000-$negativ[$a]).(50000000+$etore[$a]).(50000000+$dtore[$a]).(50000000+$a));}
      $punkte[$a]=$punkte[$a]+$strafp[$a];
      if($minus==2){$negativ[$a]=$negativ[$a]+$strafm[$a];}
      }
        ...
        if($cba>1){
          ...
          for($b=1;$b<=$cba;$b++){
            ...
            if(($abc==$anzteams-1) && ($x1==$x2)){
              ...
              if(($x3==$stat1) || ($x3==$stat2)){$def++;}
              }
            else{
              $subteams=$subteams.$x4;
              if(($x4==$stat1) || ($x4==$stat2)){$def++;}
              }
            }
          $anzcnt=0;
          if($def>0){
  ...
  if($hands==1){
    if($handp[$endtab-1]!=0){
      for($ih=0;$ih<$anzteams;$ih++){
        ...
        if($handd<10){$handd="0".$handd;}
        $taba[$jyz][$ih]=$handd.substr($taba[$jyz][$ih],2);
        }
      sort($taba[$jyz],SORT_STRING);
      }
    }
  if($sttest==true){for($x=0;$x<$anzteams;$x++){$x3=intval(substr($taba[$jyz][$x],34));$platz[$x3][$jyz]=$x+1;}}else{for($x=0;$x<$anzteams;$x++){$x3=intval(substr($taba[$jyz][$x],34));$platz[$x3][$jyz]=0;}}
    }
  }
?>