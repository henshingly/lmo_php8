...
$viewer_cache_counter_file = fopen(PATH_TO_LMO.'/'.$diroutput.'/viewer_'.$multi.'_count.txt',"rb");
...
fclose($viewer_cache_counter_file);
  ...
  if (file_exists($multi1)) {                                             // in Array lesen
    $multi_cfgarray=parse_ini_file($multi1);         // in Array lesen    // mit Hauptscript zusammenlegen
    $multi_cfgarray+=$main_cfgarray;                                     // Variablen erstellen
    extract ($multi_cfgarray);                        // Variablen erstel//                                               len
  }else{                                                                 //
...
if ($viewer_cache_counter==0 || $viewer_cache_counter > $multi_cfgarray['cache_refresh']) {
  ...
  $i=1; $output=""; $fav_liga[]=array(); $fav_team[][]=array();
