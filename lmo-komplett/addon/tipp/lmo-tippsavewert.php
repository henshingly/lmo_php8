...
// 
    ...
    while (!feof($datei)) {
      ...
      if($zeile!=""){
        if((substr($zeile,0,1)=="[") && (substr($zeile,-1)=="]")){
      	...
      	if($nick!=""){array_push($datenalt,$zeile);}
      	}
      }
    fclose($datei);
    }
  ...
  if(file_exists($file)){
    ...
    if(!isset($ende)){$ende=$anztipperliga;}

    if($lmtype!=0){$tipp_wertverein=0;}
    ...
    for($l=0;$l<$anztipperliga;$l++){  // durchlaufe alle Tipper
      ...
      if($l>=$start-1 && $l<=$ende-1){
        ...
        if(file_exists($tippfile)){
          $gef=0;
          for($g=0;$g<count($dummd) && $gef==0;$g++){ //Tipper tippt diese Liga
            $dumme = split("[|]",$dummd[$g]);
            if($tippernick1==$dumme[0]){
              fputs($auswertdatei,"\n[".$dumme[0]."]\n");
              fputs($auswertdatei,"Team=".$dumme[5]."\n");
              if($tipp_showname==1){fputs($auswertdatei,"Name=".$dumme[3]."\n");}
              if($tipp_showemail==1){fputs($auswertdatei,"Email=".$dumme[4]."\n");}
              $gef=1;
              }
            }
          if($gef==1){
            if($st==0){require(PATH_TO_ADDONDIR."/tipp/lmo-tippopenfileall.php");}
            else{require(PATH_TO_ADDONDIR."/tipp/lmo-tippopenfile.php");}

            $st1=$st;
            for($st0=0;$st0<$anzst;$st0++){
              if($st>0){$st0=$anzst;}else{$st1=$st0+1;}
              ...
              for($i=0;$i<$anzsp;$i++){
                if($lmtype==0){
                  if($tipp_jokertipp==1 && (($st==0 && $jksp[$st0]==$i+1) || $jksp==$i+1)){$jkspfaktor=$tipp_jokertippmulti;}
                  else{$jkspfaktor=1;}
                  $punktespiel=-1;
                  if($st==0){
                  ...
                  if($punktespiel>-1){$spielegetippt++;}
                  if($punktespiel>0){$tp+=$punktespiel;}
                  if($tipp_wertverein==1){
                    if($punktespiel>0){
                  ...
                  for($n=0;$n<$modus[$st-1];$n++){
                    if($tipp_jokertipp==1 && (($st==0 && $jksp[$st0]==($i+1).($n+1)) || $jksp==($i+1).($n+1))){$jkspfaktor=$tipp_jokertippmulti;}
                    else{$jkspfaktor=1;}
                    $punktespiel=-1;
                    if($st==0){
              ...
              if($spielegetippt>0){fputs($auswertdatei,"SG".$st1."=".$spielegetippt."\n");}
              if($tipp_showzus==1){
                if($tipp_tippmodus==1){
                  if($punkte1>0){fputs($auswertdatei,"P1".$st1."=".$punkte1."\n");}
                  if($punkte2>0){fputs($auswertdatei,"P2".$st1."=".$punkte2."\n");}
                  if($punkte3>0){fputs($auswertdatei,"P3".$st1."=".$punkte3."\n");}
                  if($punkte4>0){fputs($auswertdatei,"P4".$st1."=".$punkte4."\n");}
                  }
                if($punkte5>0){fputs($auswertdatei,"P5".$st1."=".$punkte5."\n");}
                if($punkte6>0){fputs($auswertdatei,"P6".$st1."=".$punkte6."\n");}
                }
              if($tp>0){fputs($auswertdatei,"TP".$st1."=".$tp."\n");}
            ...
            if($st>0){
              $dat=0;
              while($tippernick1!=substr($datenalt[$dat],1,-1) && $dat<(count($datenalt)-1)){$dat++;}
              ...
              if($st>0 && file_exists($tipp_wertvereinfile)){
                ...
                while (!feof($datei)) {
                  ...
                  if($zeile!=""){
                    ...
                    if($verein!=""){array_push($datenalt1,$zeile);}
                    }
                  }
                fclose($datei);
                }
              ...
              for($i=1;$i<=$anzteams;$i++){
                ...
                for($j=1;$j<=$anzst1;$j++){
                  if($st>0){
                    ...
                    while($datenalt1[$dat]!="[".$i."]" && $dat<(count($datenalt1)-1)){$dat++;}
                  ...
                  if($vpunkte[$i][$j]>0){fputs($tipp_wertvereindatei,"TP".$j."=".$vpunkte[$i][$j]."\n");$vpunkte[$i][$j]=0;}
                  if($vspiele[$i][$j]>0){fputs($tipp_wertvereindatei,"SG".$j."=".$vspiele[$i][$j]."\n");$vspiele[$i][$j]=0;}
                  }
                }
