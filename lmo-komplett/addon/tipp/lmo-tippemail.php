...
if($message!=""){
  ...
  if($emailart==0){
    for($tippernr=$start-1;$tippernr<$ende;$tippernr++){
      ...
      if($dummb[9]!=-1){
        ...
        if(mail($dummb[4],$subject,$textmessage,$header,$para5)){$anzemail++;}else{echo $text['tipp'][176];}
        }
      }
  ...
  elseif($emailart==1){
    $tipp_textreminder1=str_replace("\n","&#10;",$message); 
        ...
        if($dummb[10]!=-1 && $dummb[4]!=""){
          ...
          for($i=0;$i<$anzspiele;$i++){
            if($i==0 || $liga[$i]!=$liga[$i-1]){
              ...
              if(file_exists($tippfile)){require(PATH_TO_ADDONDIR."/tipp/lmo-tippemailviewer1.php");$ktipp=1;}else{$ktipp=0;}
              }
            if($ktipp==1){
              if($goaltipp[$i]=="_"){
                ...
                $spiele=$spiele.$text['tipp'][87]." ".$zeit[$i].": ".$teama[$i]." - ".$teamb[$i]."\n";
                $lliga=$liga[$i];
  ...
  elseif($emailart==2 && $adressat!=""){
    ...
    if(mail($dummb[4],$subject,$message,$header,$para5)){echo "1 ".$text['tipp'][175]."<br>";}else{echo $text['tipp'][176];}
    }
  }
?>