...
if($file!=""){
  ...
  for($a=1;$a<=$anzteams;$a++){
	...
	for($j=$hoy;$j<$endtab;$j++){
	  ...
	  if ($tabtype==4) {$endtab=($anzst/2);}
        if(($goala[$j][$i]!="_") && ($goalb[$j][$i]!="_") && ((($tabtype==0 or $tabtype==3 or $tabtype==4) && (($a==$teama[$j][$i]) || ($a==$teamb[$j][$i]))) || (($tabtype==1) && ($a==$teama[$j][$i])) || (($tabtype==2) && ($a==$teamb[$j][$i])))){
          ...
          $p0n=$pnn;
          if($spez==1){
            if($mspez[$j][$i]==$text[0]){$p0s=$pxs;$p0u=$pxu;$p0n=$pxn;}
            if($mspez[$j][$i]==$text[1]){$p0s=$pps;$p0u=$ppu;$p0n=$ppn;}
            }
          $spiele[$a]=$spiele[$a]+1;
          ...
          if(($a==$teamb[$j][$i]) && ($msieg[$j][$i]!=3)){
            ...
            elseif($msieg[$j][$i]==1){
              ...
              elseif((($goalb[$j][$i]-$goala[$j][$i])==($maxn1[$a]-$maxn2[$a])) && ($goalb[$j][$i]==$maxn1[$a])){
                $maxn0[$a]=$maxn0[$a]."<br>".$goalb[$j][$i].":".$goala[$j][$i]." ".$text[72]." ".$teams[$teama[$j][$i]]." ".$text[74];
                }
              }
            elseif($msieg[$j][$i]==0){
              if($goala[$j][$i]<$goalb[$j][$i]){
    ...
    if($kegel==0){array_push($tab0,(50000000+$punkte[$a]).(50000000-$negativ[$a]).(50000000+$dtore[$a]).(50000000+$etore[$a]).(50000000+$a));}else{array_push($tab0,(50000000+$punkte[$a]).(50000000-$negativ[$a]).(50000000+$etore[$a]).(50000000+$dtore[$a]).(50000000+$a));}
    }
  array_shift($tab0);
  ...
  if($action="admin"){$tab2=$tab0;}
  if($hands==1){
    if(($tabdat=="") || ($action=="admin")){$handb=$stt-1;}else{$handb=$endtab-1;}
    if($handp[$handb]!=0){
