...
for($i=1;$i<=count($tippdaten);$i++){
  ...
  if($op3==$st-1){ ////////////////////////////////////////////////// nur der benötigte Spieltag wird eingelesen
    ...
    if($lmtype==0){
      if(($op2=="Round") && ($op8=="GA")){
        $goaltippa[$op4]=$dum[2];
        if($goaltippa[$op4]==""){$goaltippa[$op4]=-1;}
        if($goaltippa[$op4]=="-1"){$goaltippa[$op4]="_";}
        if($goaltippa[$op4]=="-2"){$msieg[$op3][$op4]=1;$goaltippa[$op4]="0";}
        }
      if(($op2=="Round") && ($op8=="GB")){
        $goaltippb[$op4]=$dum[2];
        if($goaltippb[$op4]==""){$goaltippb[$op4]=-1;}
        if($goaltippb[$op4]=="-1"){$goaltippb[$op4]="_";}
        if($goaltippb[$op4]=="-2"){$msieg[$op3][$op4]=2;$goaltippb[$op4]="0";}
        }
      }
    else{
      if(($op2=="Round") && ($op8=="GA")){
        $goaltippa[$op6][$op7]=$dum[2];
        if($goaltippa[$op6][$op7]==""){$goaltippa[$op6][$op7]=-1;}
        if($goaltippa[$op6][$op7]=="-1"){$goaltippa[$op6][$op7]="_";}
        }
      if(($op2=="Round") && ($op8=="GB")){
        $goaltippb[$op6][$op7]=$dum[2];
        if($goaltippb[$op6][$op7]==""){$goaltippb[$op6][$op7]=-1;}
        if($goaltippb[$op6][$op7]=="-1"){$goaltippb[$op6][$op7]="_";}
        }
      } // ende else
