...
if($file!=""){
  if(substr($file,-4)==".l98"){
    ...
    if ($datei) {
      ...
      if($st>0){fputs($datei,"Actual=".$st."\n");}else{fputs($datei,"Actual=".$stx."\n");}
      if($lmtype==0){
        ...
        if(isset($namepkt)){fputs($datei,"namePkt=".$namepkt."\n");}
        if(isset($nametor)){fputs($datei,"nameTor=".$nametor."\n");}
      }else{
      ...
      for($i=1;$i<=$anzteams;$i++){
        ...
        if($lmtype==0){
          ...
          if($minus==2){fputs($datei,"SM=".$strafm[$i]."\n");}
        }
        fputs($datei,"URL=".$teamu[$i]."\n");
        fputs($datei,"NOT=".$teamn[$i]."\n");
      }
      if($lmtype!=0){$anzsp=$anzteams;}
      for($i=1;$i<=$anzst;$i++){
        fputs($datei,"\n[Round".$i."]\n");
        if($hands==1){fputs($datei,"HS=".$handp[$i-1]."\n");}
        fputs($datei,"D1=".$datum1[$i-1]."\n");
        fputs($datei,"D2=".$datum2[$i-1]."\n");
        if($lmtype!=0){
          fputs($datei,"MO=".$modus[$i-1]."\n");
          $anzsp=$anzsp/2;
          if(($klfin==1) && ($i==$anzst)){$anzsp=$anzsp+1;}
        }
        for($j=1;$j<=$anzsp;$j++){
          if(!isset($msieg[$i-1][$j-1])){$msieg[$i-1][$j-1]=0;}
          fputs($datei,"TA".$j."=".$teama[$i-1][$j-1]."\n");
          fputs($datei,"TB".$j."=".$teamb[$i-1][$j-1]."\n");
          if($lmtype==0){
            if($goala[$i-1][$j-1]=="_"){fputs($datei,"GA".$j."=-1\n");}
              elseif($msieg[$i-1][$j-1]==1){fputs($datei,"GA".$j."=-2\n");}
              else{fputs($datei,"GA".$j."=".$goala[$i-1][$j-1]."\n");}
            if($goalb[$i-1][$j-1]=="_"){fputs($datei,"GB".$j."=-1\n");}
              elseif($msieg[$i-1][$j-1]==2){fputs($datei,"GB".$j."=-2\n");}
              else{fputs($datei,"GB".$j."=".$goalb[$i-1][$j-1]."\n");}
            if($msieg[$i-1][$j-1]==3){fputs($datei,"ET".$j."=3\n");}
            if($spez==1){
              if($mspez[$i-1][$j-1]=="_"){fputs($datei,"SP".$j."=0\n");}
              elseif($mspez[$i-1][$j-1]==$text[0]){fputs($datei,"SP".$j."=2\n");}
              elseif($mspez[$i-1][$j-1]==$text[1]){fputs($datei,"SP".$j."=1\n");}
            }
            ...
            for($n=1;$n<=$modus[$i-1];$n++){
              if($goala[$i-1][$j-1][$n-1]=="_"){fputs($datei,"GA".$j.$n."=-1\n");}
                else{fputs($datei,"GA".$j.$n."=".$goala[$i-1][$j-1][$n-1]."\n");}
              if($goalb[$i-1][$j-1][$n-1]=="_"){fputs($datei,"GB".$j.$n."=-1\n");}
                else{fputs($datei,"GB".$j.$n."=".$goalb[$i-1][$j-1][$n-1]."\n");}
              if($mspez[$i-1][$j-1][$n-1]=="_"){fputs($datei,"SP".$j.$n."=0\n");}
                elseif($mspez[$i-1][$j-1][$n-1]==$text[0]){fputs($datei,"SP".$j.$n."=2\n");}
                elseif($mspez[$i-1][$j-1][$n-1]==$text[1]){fputs($datei,"SP".$j.$n."=1\n");}
