...
// 
  ...
  for($i=0;$i<$anztipper;$i++){
    ...
    if($showname==1){$tippername[$i]=$dummb[3];}
    if($showemail==1){$tipperemail[$i]=$dummb[4];}
  ...
  if($showzus==1){$punkte=array_pad($array,$anztipper+1,"");}
  ...
  for($k=0;$k<$anzligen;$k++){
    ...
    elseif(!file_exists($auswertfile1))
      ...
      for($i=1;$i<=count($tippdaten);$i++){
        ...
        elseif(substr($op4,0,1)=="P" && $showzus==1){
  ...
  for($j=0;$j<$anztipper;$j++){
    ...
    if($showname==1){fputs($auswertdatei,"Name=".$tippername[$j]."\n");}
    if($showemail==1){fputs($auswertdatei,"Email=".$tipperemail[$j]."\n");}
    for($k=1;$k<=$anzligenaus;$k++){
