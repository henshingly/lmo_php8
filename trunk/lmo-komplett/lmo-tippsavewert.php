        ...
        if(file_exists($tippfile)){
          ...
          for($g=0;$g<count($dummd) && $gef==0;$g++){ //Tipper tippt diese Liga
            ...
            if($tippernick1==$dumme[0]){
              ...
              if($showname==1){fputs($auswertdatei,"Name=".$dumme[3]."\n");}
              if($showemail==1){fputs($auswertdatei,"Email=".$dumme[4]."\n");}
              $gef=1;
              }
            }
          if($gef==1){
            ...
            for($st0=0;$st0<$anzst;$st0++){
              ...
              for($i=0;$i<$anzsp;$i++){
                if($lmtype==0){
                  if($jokertipp==1 && (($st==0 && $jksp[$st0]==$i+1) || $jksp==$i+1)){$jkspfaktor=$jokertippmulti;}
                  else{$jkspfaktor=1;}
                  $punktespiel=-1;
                  if($st==0){
                  ...
                  for($n=0;$n<$modus[$st-1];$n++){
                    if($jokertipp==1 && (($st==0 && $jksp[$st0]==($i+1).($n+1)) || $jksp==($i+1).($n+1))){$jkspfaktor=$jokertippmulti;}
                    else{$jkspfaktor=1;}
                    $punktespiel=-1;
                    if($st==0){
            ...
            if($wertverein==1){
              ...
              for($i=1;$i<=$anzteams;$i++){
                ...
                for($j=1;$j<=$anzst1;$j++){
                      ...
                      while($dat<count($datenalt1) && $verein==$i){
                        if((substr($datenalt1[$dat],0,1)=="[") && (substr($datenalt1[$dat],-1)=="]")){
                          ...
                          if(substr($datenalt1[$dat],2,strpos($datenalt1[$dat],"=")-2)!=$st){
                            fputs($wertvereindatei,$datenalt1[$dat]."\n");
                            }
                          $dat++;
                          }
                        }
                  ...
                  if($vpunkte[$i][$j]>0){fputs($wertvereindatei,"TP".$j."=".$vpunkte[$i][$j]."\n");$vpunkte[$i][$j]=0;}
                  if($vspiele[$i][$j]>0){fputs($wertvereindatei,"SG".$j."=".$vspiele[$i][$j]."\n");$vspiele[$i][$j]=0;}
                  }
                }
