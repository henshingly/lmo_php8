          ...
          if($todo!="tippuseredit" || $ftest==1 || $tipp_immeralle==1){
            ...
            if($lmotipperok>0 || $action=="admin" || $todo=="newtipper"){
              ...
              <tr><td class="lmost5"><nobr>
                <input type="radio" name="liganr" value="<? echo $i; ?>" id="<? echo $i+3; ?>" <? if(($liga=="" && $i==1) || $liga==$files){echo "checked";} ?> onClick="if(emailart[2].checked==false)changetextarea(1);emailart[2].checked=true;">
                <label for="<? echo $i+3; ?>"><? echo $t0; ?></label></nobr></td>
            ...
            <?
              ...
              if($t2==$text[2]){echo $text['tipp'][228];}else{echo $text['tipp'][229];}
              echo "</option>";
...
elseif($todo=="tippuseredit" && file_exists(PATH_TO_ADDONDIR."/tipp/".$tipp_dirtipp.substr($files,0,-4)."_".$nick.".tip")==true){echo "checked";}

if($todo=="tippoptions" && ($ftest==1 || $tipp_immeralle==1)){echo "checked";}
if($todo=="tippoptions"){echo " onClick=\"dolmoedit()\"";}
if($todo=="tippoptions" && $tipp_immeralle==1){echo " disabled";} ?>>
<? echo $t0 ?><br>
