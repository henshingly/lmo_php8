...
if(!file_exists($auswertfile))
      ...
      elseif((strpos($zeile,"=")!=false) && (substr($zeile,0,1)!=";")){
        ...
        if($schl=="Team" && $wert!=""){
          ...
          for($j=0;$j<$teamsanzahl && $gef==0;$j++){
            if($team[$j]==$wert){ // Team schonmal gefunden
              $gef=1;
              }
            }
          if($gef==0){ // Team noch nicht gefunden
            $team[$teamsanzahl]=$wert;
            $teamsanzahl++;
            }
          }
        elseif($schl=="AnzLigen"){$endtab=$wert;$anzst1=$wert;} // nur fuer die Gesamtwertung
        }
      }
    fclose($datei);
    }  
  ...
  if($tipp_showstsiege==1){$stsiege = array_pad($array,$teamsanzahl+1,"");}
  ...
  for($i=1;$i<=count($tippdaten);$i++){
    ...
    if($dum[1]=="Team"){
      if($dum[2]==""){$t=-1;} // Tipper hat kein Team
      else{
        $gef=0;
        for($j=0;$j<$teamsanzahl && $gef==0;$j++){
          if($team[$j]==$dum[2]){ // Team schonmal gefunden
            $tipp_tipperimteam[$j]++;
            $t=$j;
            $gef=1;
            }
    ...
    elseif(($stwertmodus=="nur" && ($tabdat=="" || $op3==$endtab-1)) || ($stwertmodus=="bis" && $op3<$endtab)){
      if($t>-1 && $op3>-1){
        if($op4=="TP"){$tipppunkte[$t][$op3]+=$dum[2];}
        elseif($op4=="SG"){$spielegetippt[$t][$op3]+=$dum[2];}
  ...
  if($tipp_showstsiege==1){ // Spieltagssieger ermitteln
