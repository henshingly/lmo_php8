...
if(file_exists($einsichtfile)){
  ...
  for($l=1;$l<=count($tippdaten);$l++){
    ...
    if($op4=="GB"){
      ...
      for($k=$start2;$k<=$i;$k++){
        if($spiel0==$spiel[$k]){
          if($tippa0>0){$toregesa[$k]+=$tippa0;}
          if($tippb0>=0){$toregesb[$k]+=$tippb0;$anzgetippt[$k]++;}
          
          if($tippb0<$tippa0){$tendenz1[$k]++;}
          elseif($tippb0>$tippa0){$tendenz2[$k]++;}
          elseif($tippa0>=0 && $tippb0>=0){$tendenz0[$k]++;}
          }
        }
      }
    }
