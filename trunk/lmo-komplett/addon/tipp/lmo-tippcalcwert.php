...
if(!file_exists($auswertfile))
  ...
  if($datei!=false){
    ...
    while (!feof($datei)) {
      ...
      elseif((strpos($zeile,"=")!=false) && (substr($zeile,0,1)!=";")){
        ...
        if($schl=="AnzLigen"){$endtab=$wert;$anzst1=$wert;} // nur fuer die Gesamtwertung
        elseif($sekt!="Options"){array_push($tippdaten,$sekt."|".$schl."|".$wert."|EOL");}
        }
      }
    fclose($datei);
    }  
  ...
  if($tipp_showstsiege==1){$stsiege = array_pad($array,$anztipper+1,"");}
  ...
  for($i=1;$i<=count($tippdaten);$i++){
    ...
    if($tippernick[$t]!=$op1){
      if($tippernick[$t]!=""){$t++;} 
      $tippernick[$t]=$op1;
      if($op1==$lmotippername && $op1!=""){$eigpos=$t;}
      } 
    if($dum[1]=="Team"){$tipperteam[$t]=$dum[2];}
    elseif($dum[1]=="Name"){$tippername[$t]=$dum[2];}
    elseif($dum[1]=="Email"){$tipperemail[$t]=$dum[2];}
    elseif(($stwertmodus=="nur" && ($tabdat=="" || $op3==$endtab-1)) || ($stwertmodus=="bis" && $op3<$endtab)){
      if($op4=="SG"){$spielegetippt[$t][$op3]=$dum[2];}
      elseif($op4=="TP"){$tipppunkte[$t][$op3]=$dum[2];}
      elseif($tipp_showzus==1){
        if($op4=="P1"){$punkte1[$t][$op3]=$dum[2];}
        elseif($op4=="P2"){$punkte2[$t][$op3]=$dum[2];}
        elseif($op4=="P3"){$punkte3[$t][$op3]=$dum[2];}
        elseif($op4=="P4"){$punkte4[$t][$op3]=$dum[2];}
        elseif($op4=="P5"){$punkte5[$t][$op3]=$dum[2];}
        elseif($op4=="P6"){$punkte6[$t][$op3]=$dum[2];}
        }
      }
    }

  if($tipp_showstsiege==1 && $anztipper>0){ // Spieltagssieger ermitteln
  ...
  for($a=0;$a<$anztipper;$a++){
    ...
    if($spielegetipptgesamt[$a]==""){$spielegetipptgesamt[$a]=0;}

    if($quotegesamt[$a]==""){$quotegesamt[$a]=0;}
    ...
    $tt="";
    if($gewicht=="relativ"){$tt.=(50000000+$quotegesamt[$a]);}
    elseif($gewicht=="spiele"){$tt.=(50000000+$spielegetipptgesamt[$a]);}
    $tt.=(50000000+$tipppunktegesamt[$a]);
    for($k=1;$k<=3;$k++){
      if($k==1){$tipp_krit=$tipp_krit1;}elseif($k==2){$tipp_krit=$tipp_krit2;}elseif($k==3){$tipp_krit=$tipp_krit3;}
      if($tipp_krit==0){$tt.=(50000000+$quotegesamt[$a]);}
      elseif($tipp_krit==1){$tt.=(50000000+$spielegetipptgesamt[$a]);}
      elseif($tipp_krit==6){if($tipp_showstsiege==1){$tt.=(50000000+$stsiege[$a]);}}
      elseif($tipp_showzus==1){
        if($tipp_krit==2){$tt.=(50000000+$punkte1gesamt[$a]);}
        elseif($tipp_krit==3){$tt.=(50000000+$punkte2gesamt[$a]);}
        elseif($tipp_krit==4){$tt.=(50000000+$punkte3gesamt[$a]);}
        elseif($tipp_krit==5){$tt.=(50000000+$punkte6gesamt[$a]);}
        }
      }
