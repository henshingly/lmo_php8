...
if(!file_exists($auswertfile))
  ...
  if($kurvenmodus==2 || $kurvenmodus==3){$taba = array_pad($array,$anzst+1,"");}
  if($kurvenmodus>2){$tabb = array_pad($array,$anzst+1,"");}
  
  if($kurvenmodus>1){
    for($i=0;$i<$anzst;$i++){
      if($kurvenmodus==2 || $kurvenmodus==3){$taba[$i] = array("");}
      if($kurvenmodus>2){$tabb[$i] = array("");}
      }
    }
  ...
  for($i=1;$i<=count($tippdaten);$i++){
    ...
    if($tippernick[$t]!=$op1){
      if($tippernick[$t]!=""){$t++;} 
      $tippernick[$t]=$op1;
      if($stat1==-1 && $lmotippername==$tippernick[$t]){$stat1=$t;}
      } 
    if($op4=="TP"){
    	$tipppunkte[$t][$op3]=$dum[2];
    	if($kurvenmodus>2){$tipppunktegesamt[$t][$op3]=$dum[2];}
    	}
    elseif($op4=="SG"){
    	$spielegetippt[$t][$op3]=$dum[2];
    	if($kurvenmodus>2){$spielegetipptgesamt[$t][$op3]=$dum[2];}
	}
    elseif($tipp_showzus==1){
      if($op4=="P1"){$punkte1[$t][$op3]=$dum[2];if($kurvenmodus>2){$punkte1gesamt[$t][$op3]=$dum[2];}}
      elseif($op4=="P2"){$punkte2[$t][$op3]=$dum[2];if($kurvenmodus>2){$punkte2gesamt[$t][$op3]=$dum[2];}}
      elseif($op4=="P3"){$punkte3[$t][$op3]=$dum[2];if($kurvenmodus>2){$punkte3gesamt[$t][$op3]=$dum[2];}}
      elseif($op4=="P6"){$punkte6[$t][$op3]=$dum[2];if($kurvenmodus>2){$punkte6gesamt[$t][$op3]=$dum[2];}}
      }
    }
  
  if($kurvenmodus>1 && $tipp_showstsiege==1 && ($tipp_krit1==6 || $tipp_krit2==6 || $tipp_krit3==6)){ // Spieltagssieger ermitteln
    $stsiege = array_pad($array,$anztipper+1,"");
    if($kurvenmodus>2){$stsiegegesamt = array_pad($array,$anztipper+1,"");}
    for($a=0;$a<$anztipper;$a++){
      $stsiege[$a] = array_pad(array(""),$anzst+1,"");
      if($kurvenmodus>2){$stsiegegesamt[$a] = array_pad(array(""),$anzst+1,"");}
      }
    ...
    for($i=0;$i<$anzst;$i++){
      ...
      for($a=0;$a<$anztipper;$a++){
        ...
        for($k=1;$k<=3;$k++){
          if($k==1){$tipp_krit=$tipp_krit1;}elseif($k==2){$tipp_krit=$tipp_krit2;}elseif($k==3){$tipp_krit=$tipp_krit3;}
          if($tipp_krit==-1){$tt.=50000000;}
          elseif($tipp_krit==0){$tt.=(50000000-$spielegetippt[$a][$i]);}
          elseif($tipp_krit==1){$tt.=(50000000+$spielegetippt[$a][$i]);}
          elseif($tipp_showzus==1){
            if($tipp_krit==2){$tt.=(50000000+$punkte1[$a][$i]);}
            elseif($tipp_krit==3){$tt.=(50000000+$punkte2[$a][$i]);}
            elseif($tipp_krit==4){$tt.=(50000000+$punkte3[$a][$i]);}
            elseif($tipp_krit==5){$tt.=(50000000+$punkte6[$a][$i]);}
            }
          }
        $tt.=(50000000+$a);
        array_push($tab[$i],$tt);
      ...
      if($anztipper>0){$laeng=strlen($tab[$i][0]);}
      for($a=0;$a<$anztipper;$a++){
        $x=intval(substr($tab[$i][$a],-7));
        if($tipppunkte[$x][$i]<=0){break;}
        $stsiege[$x][$i]++;
        $poswechs=1;
        for($k=0;$k<=$laeng-24;$k+=8){
          if(intval(substr($tab[$i][$a],$k+1,7))!=intval(substr($tab[$i][$a+1],$k+1,7))){break;}
      ...
      for($a=0;$a<$anztipper;$a++){
        if($kurvenmodus<4){
          ...
          for($k=1;$k<=3;$k++){
            if($k==1){$tipp_krit=$tipp_krit1;}elseif($k==2){$tipp_krit=$tipp_krit2;}elseif($k==3){$tipp_krit=$tipp_krit3;}
            if($tipp_krit==-1){$tt.=50000000;}
            elseif($tipp_krit==0){$tt.=(50000000-$spielegetippt[$a][$jyz]);}
            elseif($tipp_krit==1){$tt.=(50000000+$spielegetippt[$a][$jyz]);}
            elseif($tipp_krit==6){if($tipp_showstsiege==1){$tt.=(50000000+$stsiege[$a][$jyz]);}}
            elseif($tipp_showzus==1){
              if($tipp_krit==2){$tt.=(50000000+$punkte1[$a][$jyz]);}
              elseif($tipp_krit==3){$tt.=(50000000+$punkte2[$a][$jyz]);}
              elseif($tipp_krit==4){$tt.=(50000000+$punkte3[$a][$jyz]);}
              elseif($tipp_krit==5){$tt.=(50000000+$punkte6[$a][$jyz]);}
              }
            }
          $tt.=(50000000+$a);
        ...
        if($kurvenmodus>2){
          ...
          for($k=1;$k<=3;$k++){
            if($k==1){$tipp_krit=$tipp_krit1;}elseif($k==2){$tipp_krit=$tipp_krit2;}elseif($k==3){$tipp_krit=$tipp_krit3;}
            if($tipp_krit==-1){$tt.=50000000;}
            elseif($tipp_krit==0){$tt.=(50000000-$spielegetipptgesamt[$a][$jyz]);}
            elseif($tipp_krit==1){$tt.=(50000000+$spielegetipptgesamt[$a][$jyz]);}
            elseif($tipp_krit==6){if($tipp_showstsiege==1){$tt.=(50000000+$stsiegegesamt[$a][$jyz]);}}
            elseif($tipp_showzus==1){
              if($tipp_krit==2){$tt.=(50000000+$punkte1gesamt[$a][$jyz]);}
              elseif($tipp_krit==3){$tt.=(50000000+$punkte2gesamt[$a][$jyz]);}
              elseif($tipp_krit==4){$tt.=(50000000+$punkte3gesamt[$a][$jyz]);}
              elseif($tipp_krit==5){$tt.=(50000000+$punkte6gesamt[$a][$jyz]);}
              }
            }
          $tt.=(50000000+$a);
          ...
          if($spielegetippt[$x3][$jyz]>0){
            for(;$y>0;$y--){
              ...
              for($k=0;$k<=$laeng1-24;$k+=8){
                if(intval(substr($taba[$jyz][$y],$k+1,7))!=intval(substr($taba[$jyz][$y-1],$k+1,7))){break;}
                if($k==$laeng1-24){$poswechs=0;}
                }
              if($poswechs==1){break;}
              }
            $platz[$x3][$jyz]=$y+1;
          ...
          if($spielegetippt[$x3][$jyz]>0){
            for(;$y>0;$y--){
              ...
              for($k=0;$k<=$laeng2-24;$k+=8){
                if(intval(substr($tabb[$jyz][$y],$k+1,7))!=intval(substr($tabb[$jyz][$y-1],$k+1,7))){break;}
                if($k==$laeng2-24){$poswechs=0;}
                }
              if($poswechs==1){break;}
              }
            $platz1[$x3][$jyz]=$y+1;
