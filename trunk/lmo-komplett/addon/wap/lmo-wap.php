...
switch($_REQUEST['op']) {
  ...
  default:
    index($dirliga,$begin,$wap_anzahl_ligen_pro_seite);
    break;

}


function index($dirliga,$begin,$wap_anzahl_ligen_pro_seite)
{
  global $text;?>
  ...
  if ($wap_anzahl_ligen_pro_seite+$begin>count($dummy)) $ende=count($dummy); else $ende=$wap_anzahl_ligen_pro_seite+$begin;
...
?>
      <small>
        <a href='<?=$_SERVER['PHP_SELF'].$dirliga.$files?>&amp;op=nav&amp;st=<?=$t1?>'><?=$t0?></a>
