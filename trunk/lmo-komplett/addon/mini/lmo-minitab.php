...
/**Format of CSV-File:
  ...
  * TeamLongName|TeamShortName|TeamnameAbbrevation|Points+|Points-|Goals+|Goals-|Games|Win|Draw|Loss|Marking
  *  Teamname   | Mittelname  |  Kurzname         |Pkt.+  | Pkt.- |Tore+ | Tore-|Sp.  | + | o  | -  |Markierung
  */
if (file_exists(PATH_TO_LMO.'/'.$diroutput.$m_liga.'-tab.csv')) {
  ...
  for ($j=$nach_oben;$j<$m_anzteams-$nach_unten;$j++) {
    ...
    $template->setVariable(array("PlusTore"=>$m_tabelle[$j][5]));
    $template->setVariable(array("MinusTore"=>$m_tabelle[$j][6]));
    if (($m_diff=$m_tabelle[$j][5]-$m_tabelle[$j][6])>0) $m_diff='+'.$m_diff;
    $template->setVariable(array("Tordifferenz"=>$m_diff));
    $template->setVariable(array("Spiele"=>$m_tabelle[$j][7]));
    $template->setVariable(array("Siege"=>$m_tabelle[$j][8]));
    $template->setVariable(array("Unentschieden"=>$m_tabelle[$j][9]));
    $template->setVariable(array("Niederlagen"=>$m_tabelle[$j][10]));
    $style='';
    if ($m_tabelle[$j][11]!='') {
      if (strpos($m_tabelle[$j][11],'M')!==FALSE){  //Meister
