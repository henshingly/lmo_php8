...
if(($file!="") && ($subteams!="")){
  ...
  if ($tabtype==4) {$endtab=($anzst/2);}
    for($j=$hoy;$j<$endtab;$j++){
      for($i=0;$i<$anzsp;$i++){
        $b=0;
        for($c=0;$c<count($subteam);$c++){if($subteam[$c]==$teama[$j][$i]){$b++;}elseif($subteam[$c]==$teamb[$j][$i]){$b++;}}
        if(($b==2) && ($mcalc[$j][$i]<2) && ($goala[$j][$i]!="_") && ($goalb[$j][$i]!="_") && ((($tabtype==0 or $tabtype==3 or $tabtype==4) && (($a==$teama[$j][$i]) || ($a==$teamb[$j][$i]))) || (($tabtype==1) && ($a==$teama[$j][$i])) || (($tabtype==2) && ($a==$teamb[$j][$i])))){
          ...
          if(($a==$teama[$j][$i]) || (($a==$teamb[$j][$i]) && ($msieg[$j][$i]==3))){
            ...
            if($msieg[$j][$i]==1){
              ...
              if($minus==2){$negativ1[$a]=$negativ1[$a]+$p0n;}
              }
            elseif($msieg[$j][$i]==2){
              $nieder1[$a]=$nieder1[$a]+1;
              $punkte1[$a]=$punkte1[$a]+$p0n;
              if($minus==2){$negativ1[$a]=$negativ1[$a]+$p0s;}
              }
            elseif($msieg[$j][$i]==0){
              ...
              elseif($goala[$j][$i]==$goalb[$j][$i]){
                ...
                if($minus==2){$negativ1[$a]=$negativ1[$a]+$p0u;}
                }
              }
            }
    ...
    for($c=0;$c<count($subteam);$c++){
      if($subteam[$c]==$a){
        if($kegel==0){array_push($tab1,(50000000+$punkte1[$a]).(50000000-$negativ1[$a]).(50000000+$dtore1[$a]).(50000000+$etore1[$a]).(50000000+$a));}else{array_push($tab1,(50000000+$punkte1[$a]).(50000000-$negativ1[$a]).(50000000+$etore1[$a]).(50000000+$dtore1[$a]).(50000000+$a));}
        }
      }
    }
  array_shift($tab1);
